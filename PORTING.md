...
2. **Start from the reference firmware** in [firmware/](firmware/) if your board runs MicroPython. `robot-server.py` is the HTTP adapter, `motion.py` + `act_engine.py` are the shared 50 Hz glide engines, `channels.py` is the channel-id ↔ wire-alias table, and `PicoRobotics_gpio.py` drives servos straight off GPIO pins. On a different chip, port those ideas in whatever language fits, the protocol section 5 has per-board notes (ESP32, carrier boards, Pi, and a warning about serial-bus servos).
